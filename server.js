...
app.post('/api/usuarios/login', (req, res) => {
    ...
    conexion.query('SELECT * FROM usuarios WHERE email = ?', [email], async (err, resultados) => {
        ...
        if (match) {
            ...
            if (usuarioSeguro.foto && !usuarioSeguro.foto.startsWith('http')) {
                usuarioSeguro.foto = `${BASE_URL}/imagenes/${usuarioSeguro.foto}`;
            }
...
app.get('/api/usuarios/:id', (req, res) => {
    conexion.query(
        ...
        (err, resultados) => {
            ...
            const usuario = resultados[0];
            if (usuario.foto && !usuario.foto.startsWith('http')) {
                usuario.foto = `${BASE_URL}/imagenes/${usuario.foto}`;
            }
            res.json(usuario);
        }
    );
...
app.put('/api/usuarios/:id', upload.single('foto'), (req, res) => {
    ...
    conexion.query(query, params, (err) => {
        ...
        const fotoUrl = req.file ? `${BASE_URL}/imagenes/${req.file.filename}` : null;
        res.json({ mensaje: 'Actualizado', nombre, email, telefono, direccion, foto: fotoUrl });
    });
});
        ...
        const mailOptions = {
            ...
            html: `
                <div style="font-family:sans-serif;max-width:480px;margin:auto;padding:24px;border:1px solid #eee;">
                    ...
                    <p style="color:#999;font-size:0.85rem;">Expira en <strong>15 minutos</strong>.</p>
                </div>
            `
        };
...
app.post('/api/usuarios/restablecer-password', async (req, res) => {
    ...
    if (registro.token !== token) return res.status(400).json({ error: 'Código incorrecto' });
...
app.get('/api/productos/:id', (req, res) => {
    conexion.query('SELECT * FROM productos WHERE id = ?', [req.params.id], (err, resultados) => {
        if (err) return res.status(500).json({ error: 'Error' });
        ...
        if (producto.imagen && !producto.imagen.startsWith('http')) {
            producto.imagen = `${BASE_URL}/imagenes/${producto.imagen}`;
        }
        res.json(producto);
    });
});
...
app.get('/api/pedidos/usuario/:usuarioId', (req, res) => {
    conexion.query('SELECT * FROM pedidos WHERE usuario_id = ? ORDER BY id DESC',
        [req.params.usuarioId], (err, resultados) => {
            if (err) return res.status(500).json({ error: 'Error' });
            res.json(resultados);
        });
});
...
app.put('/api/pedidos/:id/estado', (req, res) => {
    conexion.query('UPDATE pedidos SET estado = ? WHERE id = ?',
        [req.body.estado, req.params.id], (err) => {
            if (err) return res.status(500).json({ error: 'Error' });
            res.json({ mensaje: 'Actualizado' });
        });
});
    ...
    conexion.query(
        ...
        (err, resultado) => {
            ...
            conexion.query(
                ...
                [valores],
                (errDetalle) => {
                    if (errDetalle) console.warn('Sin detalle:', errDetalle.message);
